...
function quantityForPath (path) {
  const p = String(path || '')
  if (!p) return null
  // Classify by leaf segment first: route metrics all live under
  // navigation.courseRhumbline.*, so the length leaves (crossTrackError,
  // distance) must beat the "course"/"track" angle match. velocity is a speed.
