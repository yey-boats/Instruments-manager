// proto-control.js — the yeyboats control-protocol client for the SignalK
// manager plugin.
//
// This module is the plugin's migration onto the shared `@yeyboats/proto`
...
// every outbound message is validated with `@yeyboats/proto`'s ajv validators
...
//
// `@yeyboats/proto` is an ES module; this file is CommonJS (the rest of the
...
function loadProtoLib () {
  if (!protoLibPromise) {
    protoLibPromise = import('@yeyboats/proto')
  }
...
class ProtoControl {
  // opts:
  //   controllerId  — defaults to "plugin:yey-boats-display-manager"
  ...
  constructor (opts) {
    opts = opts || {}
    this.controllerId = opts.controllerId || 'plugin:yey-boats-display-manager'
