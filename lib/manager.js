...
class YeyBoatsDisplayManager {
  ...
  async firmwareArtifactBinary (artifactId, fetchImpl) {
    ...
    // archive_download_url. There is NO sha for Actions artifacts, so we
    // cannot sha-verify — we fetch the zip (auth required, then follow the
    // 302 to the signed storage URL WITHOUT the Authorization header),
    // extract merged_firmware.bin dependency-free, and cache it.
    if (a.file.kind === 'actions-artifact') {
      ...
      if (this.app && this.app.debug) {
        this.app.debug(`yey-display TIP artifact ${artifactId}: extracted merged_firmware.bin (${binBuf.length} bytes), no sha to verify`)
      }
